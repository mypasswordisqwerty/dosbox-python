...
unsigned int MurmurFilePtr (FILE *fp)
{
    fseek(fp, 0, SEEK_END);
    int length = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    ...
    unsigned char readBlock[INCREMENTAL_READ_BLOCK];

    CMurmurHash2A *mur = new CMurmurHash2A();

    while (bytesRemaining>=(int) sizeof(readBlock))
    {
        fread(readBlock, sizeof(readBlock), 1, fp);
        mur->Add(readBlock, (int) sizeof(readBlock));
        bytesRemaining-=(int) sizeof(readBlock);
    }
    if (bytesRemaining>0)
    {
        fread(readBlock, bytesRemaining, 1, fp);
        mur->Add(readBlock, bytesRemaining);
    }
    return mur->End();
}
...
unsigned int MurmurFile (const char * filename)
{
    ...
    if (fp==0)
        return 0;
    unsigned int hash = MurmurFilePtr(fp);
    fclose(fp);
    return hash;
}
