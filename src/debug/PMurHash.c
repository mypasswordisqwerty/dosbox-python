...
uint32_t PMurHash32_File(uint32_t seed, char *filename)
{
    struct stat test;
    if (stat(filename, &test)) {
        return 0;
    }
    uint32_t length = test.st_size;
    char fbuf[length];

    /*
    std::ifstream fstream(filename, std::ios::binary);
    fstream.read(fbuf, length);
    fstream.close();
    */
    FILE *fh = fopen(filename, "rb");
    fread(fbuf, 1, length, fh);
    fclose(fh);

    return PMurHash32(seed, fbuf, length);
}
/*---------------------------------------------------------------------------*/
